...
void sparsecpu_fallback(const c10::OperatorHandle& op, torch::jit::Stack* stack, bool error_on_views) {
  ...
  for (const auto idx : c10::irange(arguments.size())) {
    ...
    if (ivalue.isTensor()) {
      ...
      (*stack)[arguments_begin + idx] = c10::IValue(c10::Device(at::kCPU));
    }
  }
  ...
  op.redispatchBoxed(c10::DispatchKeySet(c10::DispatchKey::SparseCPU), stack);
...
void xla_sparsecpu_fallback(const c10::OperatorHandle& op, torch::jit::Stack* stack) {
  XLA_FN_TRACK(3);
  const auto name = c10::toString(op.operator_name());
  ...
  // Set error_on_views as XLA should take care
  // of all view ops after functionalization.
  sparsecpu_fallback(op, stack, true);
}

TORCH_LIBRARY_IMPL(_, SparseXLA, m) {
  m.fallback(torch::CppFunction::makeFromBoxedFunction<&xla_sparsecpu_fallback>());
}
